...
export const GlobalSearchModal = ({
  ...
  const grouped = useMemo(() => {
    const posts = res.filter((r) => r.type === "post") as Extract<
      SearchResult,
      { type: "post" }
    >[];
    const categories = res.filter((r) => r.type === "category") as Extract<
      SearchResult,
      { type: "category" }
    >[];
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-xl p-0 overflow-hidden min-h-48">
        ...
        <div className="max-w-80 overflow-auto px-2 pb-4">
          {isLoading && (
          ...
          {!isLoading && res.length === 0 && query.length >= 2 && (
